<p align="center">
  <a target="_blank">
    <img src="https://miro.medium.com/v2/resize:fit:800/1*YwvcdCHDwjFgVTV-26w1DA.jpeg" alt="Clean Code Logo">
  </a>
</p>
...
adaptados para Typescript/Javascript. Isto não é um guia de estilos. É um guia para se produzir código legível, reutilizável e refatorável em Typescript.

Nem todo princípio demonstrado deve ser seguido rigorosamente, e ainda menos são um consenso universal. Estes princípios são orientações e nada mais, entretanto, foram codificados durante muitos anos de experiência coletiva pelos autores de _Código limpo_.
...
try {
  ...
  console.error(error); // Outra opção:
  notifyUserOfError(error); // Outra opção:
  reportErrorToService(error); // OU as três!
}
```
